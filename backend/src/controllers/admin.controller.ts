...
// GET /admin/kyc/queue
// Returns the full application detail so reviewers can verify identity.
// Sensitive numbers are stored/returned as last-4 only.
export async function kycQueue(_req: Request, res: Response, next: NextFunction): Promise<void> {
try {
const { rows } = await getDb().query(
`SELECT id, email, first_name, middle_name, last_name, phone, kyc_status,
        date_of_birth, ssn_last4, citizenship,
        address_street, address_unit, address_city, address_state, address_zip,
        id_type, id_last4, id_state,
        employment_status, source_of_funds, account_type_requested,
        created_at
