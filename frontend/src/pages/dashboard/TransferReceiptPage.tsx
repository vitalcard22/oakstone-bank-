...
export default function TransferReceiptPage() {
  ...
  if (isLoading) {
    return <div className="py-20 text-center text-sm text-gray-400">Loading receipt…</div>;
  }
  if (isError || !tx) {
    return (
  ...
  if (type === 'transfer') {
    ...
    rows.push(['Direction', DIRECTION_LABEL[meta?.direction] ?? meta?.direction ?? '—']);
    ...
    const addr = [r.beneficiaryStreet, r.beneficiaryCity, r.beneficiaryState, r.beneficiaryCountry].filter(Boolean).join(', ');
    if (addr || r.beneficiaryAddress) rows.push(['Beneficiary address', addr || r.beneficiaryAddress]);
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div id="receipt-printable" className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="bg-navy-700 text-white px-8 py-6 flex items-center justify-between" style={{ background: '#0f2547' }}>
          <div>
            <p className="text-xs uppercase tracking-widest opacity-70">Oakstones 1 Bank</p>
        ...
        <div className="px-8 py-6">
          ...
          <p className="text-[11px] text-gray-400 text-center mt-5">
            This receipt was generated by Oakstones 1 Bank. Reference {tx.reference_id ?? tx.id}. Keep it for your records.
          </p>
        </div>
      </div>
